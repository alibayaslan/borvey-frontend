...
const PostCard = ({
  ...
  return (
    <div id={id} className={style.workCard}>
      <a href={`/ilan/${postId}`} className={style.left}>
      ...
      <div className={style.offerWrapper}>
        <div className={style.offerInfo}>
          <img
            ...
            alt="firm logo"
          />
          <h6>{personalData.name + " " + personalData.surname}</h6>
        </div>
        <div className={style.offerBottom}>
          ...
          {status === "hold" ? (
            <div className={style.waitStatus}>
              <span>
                <ClockIcon />
              </span>
              Beklemede
          ...
          {userStatus === "online" && status === "accept" ? (
...
const MyOffers = () => {
  ...
  const getOfferData = () => {
    AxiosRequest("post", ApiRoutes.offer.myOfferlist, {
      userId: userInfo._id,
    })
      .then((res) => {
              ...
              <div className={style.workWrapper}>
                {filteredData.map((item, index) => (
                  <PostCard
                    ...
                    status={item.status}
                    userInfo={userInfo}
                    personalData={item.personalUserID}
                    postId={item.postID._id}
                    userStatus={item.personalUserID.status}
                    id={item._id}
                    acceptClick={() => {
                      responseOffer({
                        id: item._id,
                    ...
                    sendMessage={() => {
                      createMessage(userInfo._id, item.personalUserID._id);
                    }}
                  />
                ))}
              </div>
